    ...
    C: ChromBounds,
{
    ...
    pub fn merge_unchecked(&self) -> Self {
        let mut base = I::empty();
        Self::reset_base(&mut base, &self.data()[0]);
        ...
        Self::add_interval(&base, &mut cluster_intervals);
        Subtree::from_sorted(cluster_intervals)
    }
    ...
    pub fn merge_spec_strand_unchecked(&self, strand: Strand) -> Option<Self> {
        ...
        Self::add_interval(&base, &mut cluster_intervals);
        Some(Subtree::from_sorted(cluster_intervals))
    }
...
mod testing {
        ...
        C: ChromBounds,
    {
        ...
        for (c1, c2) in set.data().iter().zip(expected) {
            assert!(c1.eq(c2));
        }
    }
    ...
    fn test_merging_two_clusters() {
        ...
        let set = Subtree::from_unsorted(records);
        let merge_set = set.merge().unwrap();
        let iv1 = merge_set.data()[0];
        let iv2 = merge_set.data()[1];
