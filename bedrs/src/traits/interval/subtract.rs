...
{
    #[must_use]
    fn build_left_contained<Iv, V>(&self, other: &Iv) -> Self
    where
    ...
    #[must_use]
    fn build_right_contained<Iv, V>(&self, other: &Iv) -> Self
    where
        Iv: GenericIntervalExt<C, V>,
        V: Clone + From<T> + Into<T>,
    {
    ...
    fn build_contained_iter<Iv, V>(&self, other: &Iv) -> Box<dyn Iterator<Item = Self>>
    where
        Self: 'static,
        Iv: GenericIntervalExt<C, V>,
        V: Clone + From<T> + Into<T>,
    ...
    #[must_use]
    fn build_gt<Iv, V>(&self, other: &Iv) -> Self
    where
        Iv: GenericIntervalExt<C, V>,
        V: Clone + From<T> + Into<T>,
    {
    ...
    #[must_use]
    fn build_lt<Iv, V>(&self, other: &Iv) -> Self
    where
        Iv: GenericIntervalExt<C, V>,
        V: Clone + From<T> + Into<T>,
    {
    ...
    /// assert_eq!(s[0].end(), 20);
    /// ```
    fn subtract<Iv, V>(&self, other: &Iv) -> Option<Vec<Self>>
    where
        Iv: GenericIntervalExt<C, V>,
        V: Clone + From<T> + Into<T>,
        T: From<V>,
    ...
    fn subtract_iter<Iv, V>(&self, other: &Iv) -> Box<dyn Iterator<Item = Self>>
    where
        Self: 'static,
        Iv: GenericIntervalExt<C, V>,
        V: Clone + From<T> + Into<T>,
