use bedrs::{types::{Interval, IntervalSet, GenomicInterval, GenomicIntervalSet}, traits::{Merge, Container}};
...
pub fn merge_base(c: &mut Criterion) {
    ...
    c.bench_function("merge-base", |bench| {
        bench.iter(|| set.merge())
    });
}
...
pub fn merge_genomic(c: &mut Criterion) {
    ...
    set.sort();
    c.bench_function("merge-genomic", |bench| {
        bench.iter(|| set.merge())
    });
}
